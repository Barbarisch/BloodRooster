...
class JackDawDomainGraphIGraph(JackDawDomainGraph):
	...
	def create(dbsession, graph_id, graph_dir, sqlite_file=None):
		...
		if sqlite_file is not None:
			...
			# This is a hack.
			# Problem: using sqlalchemy to dump a large table (to get the graph data file) is resource intensive
			# Solution: if sqlite is used as the database backend we can use the sqlite3 cmdline utility to do the
			# dumping much faster
		...
		if using_sqlite_tool is False:
			...
			for ad_id in adids:
				ad_id = ad_id[0]
				t2 = dbsession.query(func.count(Edge.id)).filter_by(graph_id=graph_id).filter(EdgeLookup.id == Edge.src).filter(EdgeLookup.oid is not None).scalar()
				q = dbsession.query(Edge).filter_by(graph_id=graph_id).filter(EdgeLookup.id == Edge.src).filter(EdgeLookup.oid is not None)
	...
	def shortest_paths(self, src_sid=None, dst_sid=None, ignore_notfound=False, exclude=[], pathonly=False, maxhops=None, all_shortest=False):
		...
		try:
			...
			logger.info(f'shortest_paths finished ERR! {e}')
			traceback.print_exc()
